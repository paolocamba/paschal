    ...
    try {
        ...
        if ($service_name === 'Regular Loan Payment' || $service_name === 'Collateral Loan Payment') {
            ...
            $loan_query = "SELECT ch.LoanID, ch.PayableDate, ch.loanable_amount, ch.InterestRate, 
                          ch.PayableAmount, ch.LoanTerm, ch.ModeOfPayment
            ...
            if ($loan_result->num_rows > 0) {
                ...
                $total_owed = $loanable_amount + ($loanable_amount * ($interest_rate / 100));
                ...
                if ($remaining_balance > 0) {
                    ...
                    if ($mode_of_payment === 'Monthly') {
                        ...
                        $next_payable_amount = $payable_amount; // Same amount for each payment
                    } elseif ($mode_of_payment === 'Quarterly') {
                        $next_payable_date = date('Y-m-d', strtotime($payable_date . ' +3 months'));
                        $next_payable_amount = $payable_amount; // Same amount for each payment
                    } elseif ($mode_of_payment === 'Lump Sum') {
                        // For lump sum, next payment is the full amount
                        $next_payable_date = $payable_date;
                        $next_payable_amount = $remaining_balance;
                    } else {
                        // Default to monthly if mode not specified
                        $next_payable_date = date('Y-m-d', strtotime($payable_date . ' +1 month'));
                        $next_payable_amount = $payable_amount;
                    }
                }
                
                // Update credit_history with new payment details
                ...
                $stmt_update_credit->bind_param("dssdsi", 
                    $next_payable_amount,
                    $next_payable_date,
                    $next_payable_amount,
                    $next_payable_date,
                ...
                if ($payment_status === 'Completed' && $remaining_balance <= 0) {
