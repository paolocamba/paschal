...
<head>
    ...
    <title>Regular Loan Form 1 | Member</title>
    <!-- plugins:css -->
        ...
        <div class="main-panel">
            ...
            <br> <br> <br>
                ...
                function getMaxLoanAmount($conn, $member_id) {
                    ...
                    // If no record found, set values to 0
                    $savings = $user_data['savings'] ?? 0;
                    $share_capital = $user_data['share_capital'] ?? 0;
                    $stmt->close();
                ...
                if ($_SERVER['REQUEST_METHOD'] === 'POST') {
                    ...
                    if ($amountRequested > $maxAmount) {
                        echo "<script>
                                alert('Maximum loan amount allowed is PHP " . number_format($maxAmount, 2) . ". Please adjust your request.');
                                history.back();
                            </script>";
                        exit();
                    }
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    <script>
        $(document).ready(function () {
            ...
            const maxAmount = <?php echo $maxLoanAmount; ?>;
            const amountInput = document.getElementById('amountRequested');
            ...
            // Amount validation
            $('#amountRequested').on('input', function() {
                const value = parseFloat(this.value);
                
                if (!this.value) {
                    showValidation(this, false, 'Amount is required');
                } else if (isNaN(value) || value <= 0) {
                    showValidation(this, false, 'Please enter a valid amount');
                } else if (value > maxAmount) {
                    Swal.fire({
                        title: 'Maximum Loan Amount Notice',
                        html: `Maximum loan amount allowed is PHP ${formatNumber(maxAmount.toFixed(2))}<br>
                            This is 90% of your combined share capital and savings.`,
                        icon: 'info',
                        confirmButtonText: 'Use Maximum Amount',
                        ...
                        if (result.isConfirmed) {
                            ...
                            this.value = '';
                            showValidation(this, false, 'Please enter an amount within the maximum limit');
                        }
                    });
                } else {
                    showValidation(this, true);
            ...
            // Add info icon for maximum amount
            $('#amountRequested').after(`
                <small class="text-info" style="cursor: pointer" id="showMaxAmount">
                    <i class="fas fa-info-circle"></i> Click to see maximum amount
                </small>
            `);
